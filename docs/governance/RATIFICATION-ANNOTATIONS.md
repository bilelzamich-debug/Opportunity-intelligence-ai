...
## 15. F-W1 — Problem Weight Model: Severity and Frequency Ordinal Bands (2026-09-14)

Recorded on ratification of **F-W1**
(`decisions/F-W1-problem-weight-bands.md`), which supplies the
severity/frequency scales MISSING-12 never defined (B-37 analysed but did
not ratify them): band vocabularies, observable criteria, the rating
representation, the evidence ceiling, and the versioned weight-change
rule for backlog `T04.1.4`. Ratified by the Project Owner as six
approved decisions (WD-1–WD-6, 2026-09-14), drafted in
`platform/validation/T04.1.4-specification.md` (Rev. 2 — in which the
Project Owner first rejected the Rev. 1 corroboration-ladder WD-4 and
directed the evidence-ceiling principle). No frozen document is
rewritten; this layer records the binding interpretation (§1 precedence:
decision records → this layer → IOM → PKP v2 → backlog).

| Target | Annotation |
|---|---|
| **IOM §3.3 P-V4** | "severity and frequency present" is given its ratified scale semantics for implementation: each axis carries a structured `WeightRating` (band, detail, criterion-typed justification — F-W1 R3), and presence alone ceases to be the whole rule. P-V4's form checks: valid band, valid criteria for the axis, non-empty detail, non-empty justification, every cited fact_ref ∈ `supporting_facts`. The current detail text ("no scale defined [M-12]") describes the pre-T04.1.4 state and is superseded at implementation, not by this act. |
| **IOM §3.3 P-I4** | "Weight never asserted beyond what Facts support" receives its ordinal completion (F-W1 R4) alongside the three unchanged numeric bounds: a criterion is **defensible iff its justification entries cite Facts spanning ≥2 independence keys** (N-16 Tier-1, ACTIVE Evidence only); the **evidence ceiling** is the highest defensible criterion's band; ratings above the ceiling are violations; **below-ceiling ratings are never violations and are never upgraded**. The governing principle is binding: **Facts establish an evidence ceiling; they do not automatically determine the asserted weight** — the engine never derives a band from Fact count, Evidence count, independent-source count or evidential_support, and never infers a higher rating merely because more independent sources exist. |
| **IOM §3.3 `severity` / `frequency` + weight-revision lifecycle trigger** | The attributes' ratified content: three ordinal bands each — severity `MINOR/MODERATE/SEVERE` (criteria: recoverable friction / compounding cost / irreversible harm), frequency `EPISODIC/RECURRING/PERSISTENT` (criteria: occurrence / repetition / continuity); observable, evidence-grounded, **content-based, never count-based**; no quantitative formula; band names imply no numerical rates. "Versioned as supporting Facts accumulate or **weight is revised**" is made operational by F-W1 R5: a versioned band increase requires (1) the new rating within the evidence ceiling, (2) explicit evidence-linked justification of the higher criterion, (3) at least one additional supporting Fact in the successor cited by that justification; decreases require nothing; justification-only revisions create a new version without increasing weight. |
| **PreP1 B-37** | The recommendation "ordinal bands with evidence-linked justification" (Option 4) is adopted with the Project Owner's refinement of the evidence linkage: independent-source evidence supplies only the **uniform corroboration/sufficiency condition** — the same ≥2-key attestation rule for every band — and **never selects the band**. B-37 remains an analysis document ("no decision herein is ratified"); nothing else therein is ratified by F-W1. |
| **Backlog `T04.1.4`** | The task and its three ACs are given checkable meaning: "ordinal bands defined" = R1/R2 vocabularies and criteria; "each rating traceable to Facts" = R3 criterion-typed citations, subset-checked (the P-V5 `FactContribution` precedent applied to weight); "weight never exceeds what Facts support (P-I4)" = the R4 ceiling, enforced by the engine **before irreversible state change** (new N-10 closed-set stages `WEIGHT_EXCEEDS_EVIDENCE` and `WEIGHT_INCREASED_WITHOUT_SUPPORT`; severity evaluated before frequency; one stage per attempt; standalone path refuses before anything is written, versioned path leaves the predecessor ACTIVE) and by the P-I4 detective over stored objects. **Implementation status: NOT STARTED** — ratification changes no code and creates no tests; implementation proceeds per the specification's §9–§13 constraints (module count stays 39; store/acceptance-framework/lifecycle/contract untouched; **declaration-fidelity limitations are explicitly acknowledged and are NOT solved through NLP/LLM/network behavior** — they remain the inferer's responsibility, sampled by the F-A1 audit, with M-67 open). |
| **M-12 — partial closure (this act)** | **M-12 is PARTIALLY CLOSED by F-W1 for severity scales and frequency scales. Population scales remain OPEN** (`population_size_estimate` remains unscaled; `affected_population` remains free text) and are not addressed by this ratification. Register updated in this act: §3 Partially Closed Markers row added (by **F-W1**); §1 missing-definition counts now 23 closed / 5 partially closed / 42 open; §4 open row narrowed to the population-scales remainder; §5 forbidden-closure row 1 annotated. This row and the register update are the partial-closure record. |
| **M-19, M-20, M-67 and all other markers** | **UNCHANGED.** M-19, M-21, M-22, M-24, M-25 remain open; M-20 remains closed (F-A2); M-67 remains open with severity reduced (S-5 partial closure stands). No marker other than M-12 is affected by F-W1. |
| **S-1 / S-2 / S-4 / R-3 / N-4 / N-10 / N-16 / R-1** | **Not superseded.** Their roles are preserved and sharpened: S-1 supplies the band-definition style (observable criteria, single criterion type, worked anchors); S-2/R-3 remain the channel for corroboration → confidence (never weight); S-4's measure and floor value supply the uniform attestation boolean; N-16 Tier-1 supplies the independence keys; N-10 gains two documented closed-set extensions at implementation; N-4 is binding — the engine verifies, never invents and never upgrades; R-1/V11 govern every weight revision as a new version. |
